...
export default function Itineraries() {
  ...
  useEffect(() => {
    const fetchItineraries = async () => {
      try {
        const spotsQuery = query(collection(db, 'itineraries'), where('createdBy', '==', 'admin'));
        const snap = await getDocs(spotsQuery);
        const data = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Itinerary));
        setItineraries(data);
      } catch (error) {
    ...
    <div className="pt-24 md:pt-32 pb-16 md:pb-24 min-h-screen bg-color-paper">
      <div className="container mx-auto px-4 md:px-6 max-w-7xl">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 md:mb-16">
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-bold text-brand-ink mb-4 tracking-tight">Itineraries</h1>
          <p className="text-lg md:text-xl text-gray-500 max-w-2xl">Crafted travel plans for your perfect Sakleshpur adventure.</p>
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="py-20 text-center bg-white rounded-3xl border-2 border-dashed border-gray-200">
             <p className="text-gray-500 font-medium">No itineraries added yet.</p>
            ...
            {itineraries.map((item, i) => {
              ...
              return (
                <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                  <Card id={item.id} title={item.name} description={item.description} imageUrl={displayImage} linkTo={`/itineraries`} badge={item.duration} />
                </motion.div>
              );
            })}
          </div>
